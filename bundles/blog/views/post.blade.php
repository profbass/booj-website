...
@section('content')	
<div class="row">
	<div class="col-xs-9" id="post-page">
		@if (!empty($data['post']))
					...
					<div class="post-next-prev">
						@if (!empty($data['previous_post']) || !empty($data['next_post']) || !empty($data['']))
							@if (!empty($data['previous_post']))
								<a class="glyphicon glyphicon-previous" href="<?=$action_urls['blog']; ?>/<?=$data['previous_post']->slug; ?>" title="Previous Post">Previous</a>
							@endif

							@if (!empty($data['random_post']))
								<a class="glyphicon glyphicon-shuffle margin-left-30" href="<?=$action_urls['blog']; ?>/<?=$data['random_post']->slug; ?>" title="Random Post">Random</a>
							@endif

							@if (!empty($data['next_post']))
								<a class="glyphicon glyphicon-next margin-left-30" href="<?=$action_urls['blog']; ?>/<?=$data['next_post']->slug; ?>" title="Next Post">Next</a>
							@endif
						@endif
					</div>
			...
			@if (!empty($data['post']->user))
				<div class="post-author-section shadow-box white-box margin-bottom-30">
					...
					<h2 class="font-30">
						...
						<? if (!empty($data['post']->user->user_metadata->facebook_id)) echo '<a href="' . $action_urls['facebook'] . $data['post']->user->user_metadata->facebook_id . '" rel="nofollow" target="_blank" title="My Facebook Page"><i class="glyphicon glyphicon-red-facebook margin-top-5">Facebook</i></a>'; ?>
						<? if (!empty($data['post']->user->user_metadata->google_plus_id)) echo '<a href="' . $action_urls['google_plus'] . $data['post']->user->user_metadata->google_plus_id . '" rel="nofollow" target="_blank" title="My Google Plus Page"><i class="glyphicon glyphicon-red-google-plus margin-top-5">Google Plus</i></a>'; ?>
						<? if (!empty($data['post']->user->user_metadata->twitter_handle)) echo '<a href="' . $action_urls['twitter'] . $data['post']->user->user_metadata->twitter_handle . '" rel="nofollow" target="_blank" title="My Twitter Page"><i class="glyphicon glyphicon-red-twitter margin-top-5">Twitter</i></a>'; ?>
					</h2>
