...
class Content_Home_Controller extends Content_Base_Controller {
    ...
    public function get_homepage()
    {
		...
		$this->view_arguments['events_data'] = Tag::get_posts_in_tag_by_slug('events', 6);

		$get_pages = array('/', 'about', 'teams', 'products', 'events');
		$content_sections = array();
		$temp_js = '';
		$temp_css = '';

		foreach ($get_pages as $uri) {
			$temp = Menuitem::get_page_by_uri($uri);
			if (!empty($temp->cmspage)) {
				$temp_css .= $temp->cmspage->styles;
				$temp_js .= $temp->cmspage->scripts;
			}
			$content_sections[$uri] = $temp;
		}

		$this->view_arguments['content_js'] = $temp_js;
		$this->view_arguments['content_css'] = $temp_css;

		$this->view_arguments['content_sections'] = $content_sections;
